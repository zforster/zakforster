...
- \*ECR is not strictly required as you can use alternative registries, although ECR is the easiest and most convenient option.
...
Event-driven architectures are typically more efficient than polling or batch architectures; data is processed in near real time, the application only runs when required, and the handlers are typically smaller in scope and less complex.
...
If you require long-running compute for tasks such as executing overnight batch jobs, then Fargate is the clear choice; Fargate tasks have no timeout, they can run for as long as is necessary until processing is complete. Fargate also makes a good option if you want to shift an existing containerised application running on-prem and do not want to re-write the application to take advantage of event-driven architectures.

In contrast, Lambda is designed for shorter-lived use cases; the maximum Lambda execution duration is 15 minutes. Lambda could be a great option if you are developing a greenfield application that can take advantage of an event-driven architecture, or have small workloads that you are confident can process within the time constraints.
...
Invoked Lambdas stay **warm** for a period of time, meaning if a new request is received, an initialised environment is immediately available to process the request. If however, a request is received and no recently invoked Lambdas are available in the pool, the execution will be subject to a **cold start**. A cold start is an additional period of latency lasting between 100 ms to 2 seconds, within which AWS will initialise the lightweight Firecracker VM and pull your source code or container image. It is for this reason that it is best to keep Lambda package sizes small as the I/O influences the cold start duration that impacts the end user. To avoid cold starts it is possible to provision concurrency. This introduces additional cost, and since you are essentially running an instance at all times you may as well use Fargate as this could be an indication that it is more suited to your needs.

As Lambda is fully responsive to your traffic patterns it is a great candidate for spiky, irregular and hard to predict workloads. If no event is received, nothing will run; if a large volume of traffic arrives, it will handle the demand.

In Fargate, scaling is handled differently. A user request will be routed to an existing running service. The benefit of this is that your end user won't be impacted by a cold start; there is always a running instance immediately available to process the incoming request. New instances of your application are only created if the CPU or memory usage limits of the existing instances hit the limits defined within the Fargate service definition.

Fargate services are generally much slower to scale than Lambda-based solutions. Time to launch a new Fargate task ranges from 35 seconds to 2 minutes. A factor in this is the heavier I/O resulting from the larger image sizes permitted. As an instance is still available whilst new instances are spinning up, users are not impacted by cold starts in the same way they are with Lambda. If all running containers are at maximum capacity and another request comes in, the request will be routed to the existing task instance. Once the new instance is available, future requests can be routed to that newly available instance. As with Lambda, the running instances do not share state.

As scaling is slower to respond, a Fargate-based system requires some thought as to the minimum provisioned capacity. Fargate is a good option if you need to reduce latency, have relatively predictable traffic, and cannot suffer a cold start.
